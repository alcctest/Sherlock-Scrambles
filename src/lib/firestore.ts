...
export const getRemainingAttempts = async (email: string): Promise<number> => {
  let attempts = await db.collection('users')
      .where('email', '==', email)
      .get();
...
export const saveAttempt = async (email: string, time: number) => {
