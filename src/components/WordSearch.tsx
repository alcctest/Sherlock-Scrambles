...
export function WordSearchGame({ words, grid }: WordSearchGameProps) {
  ...
  const getCellFromEvent = useCallback(
    (e: React.MouseEvent | React.TouchEvent): [number, number] | null => {
      ...
      return row >= 0 && row < grid.length && col >= 0 && col < grid[0].length ? [row, col] : null
    },
    [grid]
  )
    ...
    <Card className="max-w-lg flex flex-col">
      ...
      <CardContent className="p-4">
        <div className="mb-4 flex flex-wrap gap-2">
          {words.map((word) => (
            <Badge key={word} variant={foundWords.includes(word) ? "secondary" : "outline"}>
              {word}
            </Badge>
          ))}
        </div>
            ...
            row.map((cell, colIndex) => (
              <div
                ...
                className={`aspect-square flex items-center justify-center cursor-pointer text-primary font-bold text-lg rounded transition-colors duration-200
                  ${isSelected(rowIndex, colIndex) ? "bg-blue-500" : isFound(rowIndex, colIndex) ? "bg-green-500" : "bg-background"}`}
